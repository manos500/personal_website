...
const Navbar = () => {
 ...
 const[toggle,setToggle] = useState(false)
  return (
    <nav className="w-full flex py-1 justify-between items-center navbar relative" id="header" >
    
      <img src={logo} alt="logo" className="w-[75px] h-[65px]" />
      <ul className="list-none sm:flex hidden justify-end items-center flex-1 relative mt-5">
      ...
      <div className={` justify-end items-center ${toggle ? 'flex' :'sm:hidden'} `}>
        <img src={menu} alt="menu"  className={`${toggle ? 'hidden' : 'flex'} w-[28px] h-[28px] object-contain cursor-pointer`} onClick={() => setToggle((prev) => !prev)}/>
         <div className={`${toggle ? 'flex' : 'hidden'} p-6  bg-gray-gradient absolute top-0 right-0  min-w-[200px]  sidebar flex-col box_shadow`}>
